...
Accessing the elements/components from the HTML structure. In that way, it can be changed/updated through the script 
while doing any simulations 
...
different colors for jobs that will enter the simulation in order to distinguish them better from one another 
...
// Logic for the bestFit Algorithm 
...
// Function to load the current Processes State
...
//Function to create the memory blocks 
...
// function to initialize coalescing 
function coalesce() {
...
// main function to coalescing or merging the memory blocks 
...
// Freeing the space after the process has finished its cycle 
...
// to confirm if the process has entered its respective memory block
...
// Resetting, restarting after a block cycles
function reset() {
